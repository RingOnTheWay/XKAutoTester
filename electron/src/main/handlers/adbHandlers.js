...
function register(ipcMain, services) {
  ...
  registerHandler(ipcMain, IPC_CHANNELS.INSTALL_APK, async (data, event) => {
    if (!adbService) {
      return { success: false, error: t('errors.adbServiceNotInit', 'ADB 服务未初始化') };
    }
    ...
    if (typeof apkPath !== 'string' || apkPath.trim() === '') {
      return { success: false, error: t('errors.invalidApkPath', '无效的 APK 路径') };
    }
    if (typeof deviceId !== 'string' || deviceId.trim() === '') {
      return { success: false, error: t('errors.invalidDeviceId', '无效的设备 ID') };
    }
    // 调用方直接持 .apkInstaller
