...
class TestCaseCodeGenerator {
  ...
  generateWaitTimeConfig(template, caseData) {
    ...
    template = template.replace(
      ...
      `APP_LOAD_WAIT_TIME = ${safeNumber(waitTimeConfig.appLoadWaitTime, 10)}`
    );
    template = template.replace(
      'ELEMENT_WAIT_TIMEOUT = 30',
      `ELEMENT_WAIT_TIMEOUT = ${safeNumber(waitTimeConfig.elementWaitTimeout, 30)}`
    );
    template = template.replace('STEP_INTERVAL = 2', `STEP_INTERVAL = ${safeNumber(waitTimeConfig.stepInterval, 2)}`);
    template = template.replace(
      'APP_CLOSE_WAIT_TIME = 2',
      `APP_CLOSE_WAIT_TIME = ${safeNumber(waitTimeConfig.appCloseWaitTime, 2)}`
    );
    return template;
  }
...
BLE_PORT = ${escapePyStringLiteral(bleDevice.port || '')}  # 蓝牙设备串口端口`;
  ...
  generateAllureAttachCode(content, name, options = {}) {
    const { type = 'TEXT', isFString = false, isVariable = false, indent = '                ' } = options;
  ...
  generateElementStepCode(step, targetApp, pagePackageData) {
    ...
    if (locatorType === 'click') {
      ...
      switch (operation) {
        case 'click':
          ...
          code += this.generateAllureAttachCode(
            `已点击坐标(${tapX}, ${tapY})${clickCount > 1 ? ` ${clickCount}次` : ''}`,
            '点击操作'
          );
          code += `                time.sleep(1)\n`;
          break;
    ...
    code += this.generateAllureAttachCode('操作失败: {str(e)}', '错误信息', {
      isFString: true,
    });
  ...
  generateMultiElementStepCode(step, targetApp, pagePackageData) {
    ...
    code += `                        \n`;
    code += this.generateAllureAttachCode(
      "操作元素: {elem_config['locator_value']}, 操作类型: {operation}",
      '元素操作',
      { isFString: true, indent: '                        ' }
    );
    code += `                        \n`;
    code += `                    except Exception as elem_error:\n`;
    code += `                        logger.error(f"操作元素失败: {elem_config['locator_value']}, 错误: {str(elem_error)}")\n`;
    code += this.generateAllureAttachCode(
      "操作元素失败: {elem_config['locator_value']}, 错误: {str(elem_error)}",
      '元素操作错误',
      { isFString: true, indent: '                        ' }
    );
    ...
    code += this.generateAllureAttachCode(
      `已从${selectedElements.length}个元素中随机选择并操作了{selected_count}个`,
      '多选元素操作',
      { isFString: true }
    );
    code += `            except Exception as e:\n`;
  ...
  generateBleStepCode(step) {
    ...
    code += this.generateAllureAttachCode('蓝牙发送数据: {hex_data}', '蓝牙操作', {
      isFString: true,
      indent: '                    ',
    });
    code += `                    time.sleep(1)\n`;
  ...
  generateSystemStepCode(step) {
    ...
    code += this.generateAllureAttachCode('screenshot', '错误截图', {
      isVariable: true,
      type: 'PNG',
    });
    code += `                pytest.fail(f"${escapePyFStringPart(step.name)}失败: {str(e)}")\n`;
  ...
  generatePageStepCode(step, steps, pagePackageData) {
    ...
    if (operationType === 'search') {
      ...
      const { locatorType: searchLocator, locatorValue: searchLocatorValue } = this._resolveLocator(
        searchConfig,
        pagePackageData
      );
      ...
      if (searchType === 'text') {
        ...
        const xpathExpr =
          matchType === 'exact' ? `//*[@text="${escapedTextValue}"]` : `//*[contains(@text, "${escapedTextValue}")]`;
      ...
      const hasTolerance =
        compareConfig.tolerance !== undefined && compareConfig.tolerance !== null && compareConfig.tolerance !== '';
      const isRandomRangeTarget = targetValueType === 'ble';

      // 优先从最新的页面封装数据中获取元素定位信息
      const { locatorType: compareLocator, locatorValue: compareLocatorValue } = this._resolveLocator(
        compareConfig,
        pagePackageData
      );
      ...
      code += `                screenshot = self.driver.get_screenshot_as_png()\n`;
      code += this.generateAllureAttachCode('screenshot', '截图', {
        isVariable: true,
        type: 'PNG',
      });
