...
class TestCaseService {
    ...
    async saveTestCase(caseData) {
        try {
            ...
            // 生成ID和时间戳
            if (!caseData.id) {
                caseData.id = `tc_${Date.now().toString(36)}_${Math.random().toString(36).substring(2, 10)}`;
            }
            ...
            // 确保文件名有效
            let fileName = caseData.fileName || 'test_case';
            fileName = fileName.replace(/[^a-zA-Z0-9_\u4e00-\u9fa5]/g, '_');
            if (!fileName.startsWith('test_')) {
                fileName = `test_${fileName}`;
            }
            caseData.fileName = fileName;
        ...
        try {
            ...
            if (jsonExists) {
                try {
                    ...
                    if (!pyFilePath) {
                        pyFilePath = caseData.pyFilePath || null;
                    }
                    
                    if (!pyFilePath && caseData.pyOutputDir) {
                        pyFilePath = path.join(caseData.pyOutputDir, `${fileName}.py`);
                    }
                } catch (e) {
                    // JSON 读取失败，继续删除流程
                }
            ...
            if (pyFilePath) {
                try {
                    await fs.access(pyFilePath);
                    await fs.unlink(pyFilePath);
                } catch (e) {
                    // .py 文件不存在，忽略
                }
            } else if (!jsonExists) {
...
BLE_PORT = "${bleDevice.port || ''}"  # 蓝牙设备串口端口`;
    ...
    async checkPyFileExists(caseData) {
        let pyFilePath = caseData.pyFilePath || null;
        if (!pyFilePath && caseData.pyOutputDir) {
            pyFilePath = path.join(caseData.pyOutputDir, `${caseData.fileName}.py`);
        }
    ...
    async cleanupOrphanedFiles() {
        ...
        for (const jsonFile of jsonFiles) {
            ...
            try {
                ...
                let pyFilePath = caseData.pyFilePath;
                if (!pyFilePath && caseData.pyOutputDir) {
                    pyFilePath = path.join(caseData.pyOutputDir, `${fileName}.py`);
                }

                if (pyFilePath) {
                    const pyExists = await fs.access(pyFilePath).then(() => true).catch(() => false);
                    if (!pyExists) {
                        await fs.unlink(jsonPath);
        ...
        for (const caseData of parsedCases) {
            ...
            const pyFiles = await fs.readdir(outputDir);
