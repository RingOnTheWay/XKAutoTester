...
class UserDataMigrator {
  ...
  async migrateConfigToNewPath(oldPath, newPath) {
    ...
    // P3-3: 抽公共复制 (原与 migrateDataToPath 复制粘贴式双份)
    this._copyUserFiles(oldConfigPath, newConfigPath);
  ...
  /**
   * P3-3: 公共复制逻辑 (userFiles 文件 + userDirs 目录)
   * @param {string} srcConfigDir
   * @param {string} dstConfigDir
   */
  _copyUserFiles(srcConfigDir, dstConfigDir) {
    for (const file of this.userFiles) {
      const src = path.join(srcConfigDir, file);
      const dst = path.join(dstConfigDir, file);
      if (fs.existsSync(src)) {
        fs.copyFileSync(src, dst);
      }
    }
    ...
    for (const dir of this.userDirs) {
      const srcDir = path.join(srcConfigDir, dir);
      const dstDir = path.join(dstConfigDir, dir);
  ...
  async migrateDataToPath(newPath) {
    ...
    // P3-3: 复用公共复制
    this._copyUserFiles(this.userConfigPath, newConfigDir);
