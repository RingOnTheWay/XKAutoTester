...
// R24 P2-10: 步骤渲染域拆分 (renderSteps → renderPageConfig)
import { stepsRenderer } from './modules/stepsRenderer.js';
    ...
    cards.forEach((card, index) => {
      ...
      if (multiElementItems.length > 0) {
        ...
        multiElementItems.forEach((item) => {
          ...
          // R24 P1-1: 其余 multi select 字段统一走 applySelectRoute (与写方向同一 schema),
          // 输入类字段收敛到 operationValue 下 — 修复写/渲染/收集三处字段路径漂移导致配置保存后失效
          const itemIndex = Number(item.dataset.index);
          item.querySelectorAll('.custom-select[data-select-id^="tc-multi-"]').forEach((sel) => {
            const selectId = sel.dataset.selectId;
            if (selectId === 'tc-multi-element-select' || selectId === 'tc-multi-operation-select') return;
            const value = getSelectedValue(sel);
            if (value) applySelectRoute(config, selectId, value, itemIndex);
          });
          // 非 select 输入字段统一 operationValue 下 (与渲染回填 renderMultiOperationValue 对齐)
          const opValueObj = (elem.operationValue = elem.operationValue || {});
          const customInput = item.querySelector('.tc-multi-custom-input');
          if (customInput) opValueObj.inputValue = customInput.value;
          const randomMin = item.querySelector('.tc-multi-random-min');
          const randomMax = item.querySelector('.tc-multi-random-max');
          if (randomMin || randomMax) {
            opValueObj.randomConfig = opValueObj.randomConfig || {};
            if (randomMin) opValueObj.randomConfig.minValue = parseFloat(randomMin.value) || 0;
            if (randomMax) opValueObj.randomConfig.maxValue = parseFloat(randomMax.value) || 100;
          }
...
// R24 P2-10: 步骤渲染域方法挂载到 prototype (this 不变, 行为零变化)
Object.assign(TestCaseView.prototype, stepsRenderer);
