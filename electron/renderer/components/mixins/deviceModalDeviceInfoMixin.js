...
export const deviceModalDeviceInfoMixin = {
  ...
  async getDeviceInfo(deviceId) {
    try {
      ...
      const manufacturerResult = await window.electronAPI.executeAdbCommand(
      ...
      // Android版本
      const androidVersionResult = await window.electronAPI.executeAdbCommand(
  ...
  resetDeviceStatusCard() {
    const elements = ['modal-device-manufacturer', 'modal-device-model', 'modal-device-android-version'];
    elements.forEach((id) => {
