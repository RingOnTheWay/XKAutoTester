<!--
  ...
  复制自 release-body-template.md, 填写 0.1.6-dev.2 的实际值。
  用法: 运行 publish-release.ps1 -Version 0.1.6-dev.2, 随后 patch-body.ps1 -Version 0.1.6-dev.2
-->

# XKAutoTester v0.1.6-dev.2
...
| 安装包                                        | 大小       | 说明                                                             |
| ------------------------------------------ | -------- | -------------------------------------------------------------- |
| `XKAutoTester.Setup.v0.1.6-dev.2.exe`      | 362.3 MB | **完整版**：内置 Python 3.12 + Android SDK + scrcpy + CP210x 驱动，开箱即用 |
| `XKAutoTester.Lite.Setup.v0.1.6-dev.2.exe` | 122.4 MB | **精简版**：不含内置环境，适合已自备 Python/JDK/ADB 的用户                        |

## 🔐 完整性校验（在线更新依赖，安装前请核对）

**XKAutoTester.Setup.v0.1.6-dev.2.exe**
SHA256: `（待版本定稿后填写）`

**XKAutoTester.Lite.Setup.v0.1.6-dev.2.exe**
SHA256: `（待版本定稿后填写）`

## ✨ 新增

- **文件选择器记住上次选择路径**：选择测试目录 / 选择文件 / 文件上传 / 选择 APK / 配置导入 / 配置导出共 6 个选择器，默认定位到上次选择的路径（目录→自身，文件→父目录，路径已删除自动回退），持久化到 config.json，跨会话生效。
- **全量代码审查修复收尾**：对主进程 / 渲染层 / Python 核心进行全量代码审查，完成 P1×8 + P2×11 + P3×16 全部整改，并补齐工程化基建。
- **其他杂项**：multi 元素步骤字段收敛、ADB 系统路径防护、生成器路径清洗、pytest 看门狗修复、confirm 弹窗全仓唯一收敛、i18n key 补齐与完整性测试、CI 强制化、view.js 步骤渲染域拆分等。
...
- **文档同步**：AGENTS.md 版本/技术栈/新模块记录更新至当前（0.1.6-dev.2 / Vite 7.3.6 / electron-vite 5.0.0）。
...
- **更新下载取消真中止**：下载中点"取消"/叉掉更新弹窗原仅关闭窗口、主进程后台继续下载（会静默下完整个安装包）；现取消会真正中止下载并清除临时文件，且弹出"已取消下载"提示；下载完成后的就绪态点取消则仅关窗（推迟安装）。

- **dev / npm start 启动无窗口修复**：R25 在入口加的 `require.main === module` 守卫在 electron-vite dev 下恒不成立（Electron 的 `require.main` 指向 electron 可执行本身）→ 启动逻辑被整体跳过、进程存活但无窗口无日志；现改判 `process.versions.electron`，dev 与 npm start 均正常启动（打包版不受影响）。
