...
const USER_DATA_MIGRATOR_PATH = path.join(PROJECT_ROOT, 'electron', 'src', 'main', 'services', 'UserDataMigrator.js');
...
test.before(() => {
  ...
  if (!_existed) {
    ...
    fs.writeFileSync(
      TEMPLATE_PATH,
      JSON.stringify({ APP_SETTINGS: { autoCheckUpdate: true }, LOG_CONFIG: { level: 'INFO' } }, null, 2),
      'utf8'
    );
  }
});
...
test('config/config.json autoCheckUpdate = true', () => {
  ...
  assert.strictEqual(config.APP_SETTINGS.autoCheckUpdate, true, 'autoCheckUpdate 必须为 true (与 AGENTS.md 文档一致)');
});
...
test('UserDataService.js 不再硬编码 config.json 副本', () => {
  ...
  assert.ok(!/LOG_CONFIG:\s*{\s*level:/.test(content), '不应硬编码 LOG_CONFIG 副本');
  assert.ok(!/SCRCPY_PARAMS:\s*{\s*max_size:/.test(content), '不应硬编码 SCRCPY_PARAMS 副本');
});
...
test('UserDataMigrator.js _getDefaultConfig 从模板文件读取', () => {
  ...
  assert.ok(/_getDefaultConfig[\s\S]*?return\s*{}/.test(content), '_getDefaultConfig 读取失败时应返回 {} 容错');
});

test('config.py 不再硬编码 DEFAULT_CONFIG', () => {
  const content = fs.readFileSync(CONFIG_PY_PATH, 'utf8');
  assert.ok(!/^DEFAULT_CONFIG\s*=/m.test(content), 'config.py 不应再有 DEFAULT_CONFIG 模块级常量');
  assert.ok(
...
test('config.py _load_config 失败时抛 FileNotFoundError', () => {
  const content = fs.readFileSync(CONFIG_PY_PATH, 'utf8');
  assert.ok(/_load_config[\s\S]*?FileNotFoundError/.test(content), '_load_config 文件不存在时应抛 FileNotFoundError');
});

test('config.py _detect_config_path 支持 XKAUTOTESTER_USER_DATA 环境变量', () => {
  const content = fs.readFileSync(CONFIG_PY_PATH, 'utf8');
  assert.ok(/XKAUTOTESTER_USER_DATA/.test(content), '_detect_config_path 应优先读取 XKAUTOTESTER_USER_DATA 环境变量');
});
