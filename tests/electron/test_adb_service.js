...
const { createSpawnMock, createExecMock, setupChildProcessMock } = require(
  path.join(__dirname, 'helpers', 'serviceMock.js')
);
...
test('getConnectedDevices 区分 device/unauthorized/offline 状态', async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(
      devices.find((d) => d.id === 'dev1'),
      { id: 'dev1', status: 'device' }
    );
    assert.deepStrictEqual(
      devices.find((d) => d.id === 'dev2'),
      { id: 'dev2', status: 'unauthorized' }
    );
    assert.deepStrictEqual(
      devices.find((d) => d.id === 'dev3'),
      { id: 'dev3', status: 'offline' }
    );
  } finally {
    restoreElectron();
  }
...
test('getConnectedDevices daemon 冷启动慢: 轮询重试直到成功', async () => {
  ...
  try {
    ...
    assert.deepStrictEqual(callLog, [['devices'], ['start-server'], ['devices'], ['devices'], ['devices']]);
    assert.strictEqual(devices.length, 2);
  } finally {
    restoreElectron();
  }
...
test('getConnectedDevices start-server 协议错误: kill-server 重建后成功', async () => {
      ...
      if (cmd === 'start-server') {
        ...
        if (callLog.filter((a) => a[0] === 'start-server').length === 1) {
          return {
            success: false,
            output: '',
            error: "adb.exe: failed to check server version: protocol fault (couldn't read status): connection reset",
          };
        }
        return { success: true, output: '* daemon started successfully', error: '' };
      }
  ...
  try {
    ...
    assert.deepStrictEqual(callLog, [['devices'], ['start-server'], ['kill-server'], ['start-server'], ['devices']]);
    assert.strictEqual(devices.length, 1);
...
test('ADBService 通过 pathHelper.getAdbPath 获取 adb 路径', async () => {
  ...
  try {
    ...
    assert.strictEqual(typeof svc.getAdbPath, 'undefined', 'ADBService 不应再有 getAdbPath 方法 (已委托 pathHelper)');
...
test('P1-6 renameRemoteFile 同目录改名: mv + 新路径拼接', async () => {
  ...
  try {
    ...
    await new Promise((resolve) => setTimeout(resolve, 10));
    assert.ok(
      capturedArgs && capturedArgs.includes('mv'),
      `capturedArgs 应为 mv args, 实际: ${JSON.stringify(capturedArgs)}`
    );
