...
const ApkInstaller = require(
  path.join(__dirname, '..', '..', 'electron', 'src', 'main', 'services', 'adb', 'ApkInstaller.js')
);

// ── mock 工厂 ──────────────────────────────────────────────

function createMockSpawn(opts = {}) {
  const { stdoutChunks = [], stderrChunks = [], code = 0 } = opts;
...
test('install push 失败返回 success=false + monitor emit error', async () => {
  ...
  assert.ok(
    executorCalls[0].args.some((a) => a.includes('temp_')),
    'cleanup 目标应为 temp_*.apk'
  );
