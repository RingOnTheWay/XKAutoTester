...
function makeFakeFileSystem(opts = {}) {
  ...
  const defaultExists =
    opts.defaultExists !== undefined ? opts.defaultExists : false;
  ...
  return {
    ...
    createWriteStream: (p) => {
      calls.createWriteStream.push(p);
      return {};
    },
  };
}
...
function makeFakeDownloadStrategy(result = null, error = null) {
  ...
  return {
    ...
    async download(downloadUrl, filePath, eventSender) {
      ...
      if (error) throw error;
      return (
        result || { success: true, filePath, message: "Download completed" }
      );
    },
    cancelDownload() {
      calls.cancel++;
...
function makeFakeApp(opts = {}) {
  const versionService = makeFakeVersionService(
    opts.version || "1.0.0",
    opts.fullVersion,
  );
  const userDataService = makeFakeUserDataService(
    opts.configPath || "/fake/config",
  );
  const fileSystem = makeFakeFileSystem(opts.fileSystem || {});
  const updateSource = makeFakeUpdateSource(
    opts.release || null,
    opts.updateSourceError || null,
  );
  const downloadStrategy = makeFakeDownloadStrategy(
    opts.downloadResult || null,
    opts.downloadError || null,
  );
  const installStrategy = makeFakeInstallStrategy(
    opts.installResult || null,
    opts.installError || null,
  );
  const versionComparatorCalls = { compare: [] };
  const versionComparator =
    opts.versionComparator ||
    ((v1, v2) => {
...
function makeRelease(opts = {}) {
  return {
    ...
    assets: opts.assets || [
      {
        name: "XKAutoTester Setup v2.0.0.exe",
        browser_download_url:
          "https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/2.0.0.exe",
        size: 50000000,
      },
    ],
  };
...
test("constructor 收 5 factory + 5 实例建 + _initialized=false", () => {
  const { svc, fileSystem, updateSource, downloadStrategy, installStrategy } =
    makeFakeApp();
  ...
  assert.strictEqual(
    svc._downloadStrategy,
    downloadStrategy,
    "downloadStrategy 实例建",
  );
  assert.strictEqual(
    svc._installStrategy,
    installStrategy,
    "installStrategy 实例建",
  );
  assert.strictEqual(svc._fileSystem, fileSystem, "fileSystem 实例建");
  assert.strictEqual(
    typeof svc._versionComparator,
    "function",
    "versionComparator 注入",
  );
  // 懒初始化: constructor 不触发 fs
  assert.strictEqual(
    fileSystem.calls.ensureDir.length,
    0,
    "constructor 不调 ensureDir",
  );
  assert.strictEqual(
    fileSystem.calls.readdir.length,
    0,
    "constructor 不调 readdir",
  );
});
...
test("懒初始化: constructor 不触发 fs, 首次 downloadUpdate 触发 ensureDir + cleanupOldUpdates", async () => {
  ...
  assert.strictEqual(
    fileSystem.calls.ensureDir.length,
    0,
    "constructor 后 ensureDir 未调",
  );
  assert.strictEqual(
    fileSystem.calls.readdir.length,
    0,
    "constructor 后 readdir 未调",
  );
  ...
  assert.strictEqual(
    fileSystem.calls.ensureDir.length,
    1,
    "首次 downloadUpdate 调 ensureDir 1 次",
  );
  assert.strictEqual(
    fileSystem.calls.ensureDir[0],
    svc.updateDir,
    "ensureDir 收 updateDir",
  );
  assert.ok(
    fileSystem.calls.readdir.length >= 1,
    "首次 downloadUpdate 触发 cleanupOldUpdates readdir",
  );
  // cleanupOldUpdates 应删除 .exe 文件 (old-installer.exe, 可能还有 setup.exe 本身因 defaultExists=true)
  assert.ok(
    fileSystem.calls.unlink.length >= 1,
    "cleanupOldUpdates 删除 old-installer.exe",
  );
...
test("懒初始化幂等: 重复 downloadUpdate 仅初始化一次", async () => {
  ...
  assert.strictEqual(
    fileSystem.calls.ensureDir.length,
    1,
    "3 次 downloadUpdate 仅 ensureDir 1 次",
  );
});
...
test("checkForUpdate 调 updateSource.fetchLatestRelease + versionComparator", async () => {
  ...
  assert.strictEqual(
    updateSource.calls.fetchLatestRelease,
    1,
    "fetchLatestRelease 调 1 次",
  );
  assert.strictEqual(
    versionComparatorCalls.compare.length,
    1,
    "versionComparator 调 1 次",
  );
  assert.deepEqual(versionComparatorCalls.compare[0], {
    v1: "1.0.0",
    v2: "2.0.0",
  });
...
test("checkForUpdate 无 release 返 hasUpdate=false + currentVersion=latestVersion", async () => {
  ...
  assert.strictEqual(
    ...
    "1.0.0",
    "无 release 时 latestVersion=currentVersion",
  );
});
...
test("checkForUpdate 同版本不误报: 本地 fullVersion 与 tag 相同 → hasUpdate=false", async () => {
  ...
  assert.strictEqual(
    result.hasUpdate,
    false,
    "同版本 (fullVersion==tag) → hasUpdate=false",
  );
...
test("checkForUpdate versionComparator 收到 fullVersion 而非 version", async () => {
  ...
  assert.strictEqual(result.hasUpdate, false);
  assert.deepEqual(
    versionComparatorCalls.compare[0],
    { v1: "1.0.5-dev.2", v2: "1.0.5-dev.2" },
...
test("checkForUpdate 更高 pre-release 版本仍检测到更新", async () => {
  ...
  assert.strictEqual(
    result.hasUpdate,
    true,
    "1.0.5-dev.2 < 1.0.6-dev.1 → hasUpdate=true",
  );
});
...
test("downloadUpdate 已存在文件返快路径 {success, filePath, message}", async () => {
  ...
  assert.strictEqual(
    downloadStrategy.calls.download.length,
    0,
    "快路径不调 downloadStrategy",
  );
});
...
test("downloadUpdate 不存在调 downloadStrategy.download", async () => {
  ...
  assert.strictEqual(
    downloadStrategy.calls.download.length,
    1,
    "downloadStrategy.download 调 1 次",
  );
...
test("installUpdate 调 installStrategy.install (fileSystem.exists=true)", async () => {
  ...
  assert.strictEqual(result.success, true);
  assert.strictEqual(
    installStrategy.calls.install.length,
    1,
    "installStrategy.install 调 1 次",
  );
  assert.strictEqual(installStrategy.calls.install[0], filePath);
  assert.ok(
    fileSystem.calls.exists.includes(filePath),
    "installUpdate 检查 existsSync",
  );
});
...
test("installUpdate 文件不存在抛 Failed to install update: Update file not found", async () => {
  ...
  await assert.rejects(
    ...
    (err) => {
      assert.strictEqual(
        err.message,
        "Failed to install update: Update file not found",
      );
      return true;
    },
  );
...
test("constructor allowInsecureSSL 默认 false: _httpsAgent=undefined, factory 收 undefined", () => {
  ...
  const svc = new UpdateService(
    makeFakeVersionService(),
    makeFakeUserDataService(),
    {
...
test("constructor allowInsecureSSL=true: _httpsAgent 非 undefined + rejectUnauthorized=false, factory 收 agent", () => {
  ...
  assert.ok(
    svc._httpsAgent instanceof https.Agent,
    "_httpsAgent 是 https.Agent 实例",
  );
  assert.strictEqual(
    svc._httpsAgent.options.rejectUnauthorized,
    false,
...
} = require("../../electron/src/main/services/application/factories");
...
test("defaultUpdateServiceFactory 纯构造: 不读 config, allowInsecureSSL=false (M4)", () => {
  ...
  const svc = defaultUpdateServiceFactory(
    { __tag: "version" },
    userDataService,
  );
...
} = require("../../electron/src/main/services/application/effects");
...
test("defaultUpdateServiceInitializer 读 config.json allowInsecureSSL=true → 调 initialize", async () => {
  ...
  try {
    ...
    const svc = new UpdateService(
      makeFakeVersionService(),
      { getUserConfigPath: () => tmpDir },
      {
        updateSourceFactory: spy,
      },
    );
    assert.strictEqual(
      svc._allowInsecureSSL,
      false,
      "initializer 前默认 false",
    );
...
test("defaultUpdateServiceInitializer config 无 allowInsecureSSL key → 保持 false", async () => {
  ...
  try {
    ...
    const svc = new UpdateService(makeFakeVersionService(), {
      getUserConfigPath: () => tmpDir,
    });
...
function makeAppWithSha256(opts = {}) {
  ...
  const actualHash =
    opts.actualHash !== undefined ? opts.actualHash : expectedHash;
  ...
  const app = makeFakeApp({
    ...
    fileSystem: {
      defaultExists: false,
      readdirResult: [],
      existsResults: opts.existsResults || {},
    },
  });
...
test("parseSha256FromBody 解析 SHA256 行 (64位 hex)", () => {
  const hash =
    "a3f5b8c1d2e4f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1";
...
test("parseSha256FromBody 按 fileName 匹配 asset 专属 hash (完整包)", () => {
  ...
  assert.strictEqual(
    parseSha256FromBody(body, fullFileName),
    fullHash,
    "完整包 fileName 匹配完整包 hash",
  );
  assert.strictEqual(
    parseSha256FromBody(body, liteFileName),
    liteHash,
    "Lite 包 fileName 匹配 Lite 包 hash",
  );
});
...
test("parseSha256FromBody fileName 不匹配回退首个 SHA256", () => {
  ...
  assert.strictEqual(
    parseSha256FromBody(body, "nonexistent.exe"),
    hash,
    "fileName 不匹配回退首个",
  );
});
...
test("parseSha256FromBody fileName 含正则特殊字符 (.exe 的 .) 正确转义", () => {
  ...
  const body = `**${fileName}**\nSHA256: ${hash}\n`;
  assert.strictEqual(
    parseSha256FromBody(body, fileName),
    hash,
    ". 正确转义, 按名匹配",
  );
});
...
test("parseSha256FromBody 无 fileName 回退首个 (向后兼容)", () => {
  ...
  assert.strictEqual(
    parseSha256FromBody(body, undefined),
    hash,
    "fileName=undefined 取首个",
  );
  assert.strictEqual(
    parseSha256FromBody(body, ""),
    hash,
    "fileName=空串取首个",
  );
});
...
test("installUpdate 安装前 SHA256 不匹配 → 抛错 + 不调 installStrategy", async () => {
  ...
  await assert.rejects(svc.installUpdate(filePath), /SHA256 校验失败/);
...
test("R10 installUpdate 无 _expectedSha256 → 拒绝安装 + missing_hash code", async () => {
  ...
  await assert.rejects(svc.installUpdate(filePath), (err) => {
...
test("R10 checkForUpdate 无 release → result.secure=false (无更新场景)", async () => {
  ...
  assert.strictEqual(
    result.secure,
    false,
    "无 release 时 secure=false (无更新不安装)",
  );
});
...
test("P0-4 sanitizeUpdateFileName: 剥离路径成分 + 扩展名白名单", () => {
  const { sanitizeUpdateFileName } = require(
    path.join(
      __dirname,
      "..",
      "..",
  ...
  assert.strictEqual(
    sanitizeUpdateFileName("XKAutoTester Setup v2.0.0.exe"),
    "XKAutoTester Setup v2.0.0.exe",
  );
  assert.strictEqual(
    sanitizeUpdateFileName("..\\..\\victim.exe"),
    "victim.exe",
    "路径穿越必须被裁剪为 basename",
  );
  assert.strictEqual(
    sanitizeUpdateFileName("..\\\\..\\\\evil.exe"),
    "evil.exe",
  );
  assert.strictEqual(sanitizeUpdateFileName("setup.zip"), "setup.zip");
  assert.strictEqual(
    sanitizeUpdateFileName("setup.pdf"),
    null,
    "非白名单扩展名拒绝",
  );
  ...
  assert.strictEqual(
    sanitizeUpdateFileName("evil.exe\u0000.txt"),
    null,
    "控制字符拒绝",
  );
});
...
test("P0-4 isTrustedDownloadUrl: 仅放行 GitHub release 域", () => {
  const { isTrustedDownloadUrl } = require(
    path.join(
      __dirname,
      "..",
      "..",
  ...
  assert.strictEqual(
    isTrustedDownloadUrl("https://objects.githubusercontent.com/xxx/app.exe"),
    true,
  );
...
test("P0-4 downloadUpdate 路径穿越文件名被裁剪为 basename", async () => {
  ...
  assert.ok(
    call.filePath.endsWith("victim.exe"),
    "应使用裁剪后的文件名 (basename)",
  );
});
...
test("P3-1 deleteUpdateFile 越界路径 (.. 回溯) → 拒绝 + 不 unlink", async () => {
  ...
  const result = await svc.deleteUpdateFile(
    path.join("/fake/config", "updates", "..", "..", "victim.exe"),
  );
...
test("P3-2 两次 checkForUpdate 不同 asset → 各自 hash 按文件名保留", async () => {
    ...
    assets: [
      {
        name: "XKAutoTester Setup v2.0.0.exe",
        browser_download_url:
          "https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/full.exe",
        size: 1,
      },
    ],
    body: `Release notes\nSHA256: ${hashFull}`,
  ...
  const releaseLite = makeRelease({
    assets: [
      {
        name: "XKAutoTester Setup v2.0.0 Lite.exe",
        browser_download_url:
          "https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/lite.exe",
        size: 1,
      },
    ],
    body: `Release notes\nSHA256: ${hashLite}`,
...
test("R27 cancelDownload 代理到 downloadStrategy", async () => {
  ...
  assert.strictEqual(
    downloadStrategy.calls.cancel,
    1,
    "service 代理应调 strategy.cancelDownload",
  );
});
...
test("R27 normalizeUpdateError fetch cause 内 SSL 错误 → ssl_failed (非 unknown)", () => {
  ...
  raw.cause = {
    errors: [
      { code: "UNABLE_TO_VERIFY_LEAF_SIGNATURE", message: "unable to verify" },
    ],
  };
  const result = normalizeUpdateError(raw);
...
// ── R27: 显示带 v 前缀 (latestVersionDisplay 与 tag_name 一致) ────────────────

test("R27 checkForUpdate 返回 latestVersionDisplay 带 v (UI 显示用)", async () => {
  const release = makeRelease({ tag_name: "v0.1.6-dev.2" });
  const { svc } = makeFakeApp({
    version: "0.1.6-dev.1",
    release,
    // 默认假 comparator 不识别 dev.1/dev.2 (返 0=等于); 自定义返 dev.2 更新
    versionComparator: (v1, v2) => -1,
  });
  const result = await svc.checkForUpdate();
  assert.strictEqual(result.hasUpdate, true);
  // 显示保留 v: UI 弹窗应展示 'v0.1.6-dev.2' (与 GitHub Release 名称统一)
  assert.strictEqual(result.latestVersionDisplay, "v0.1.6-dev.2");
  // 比较用剥 v (semver 不接受 v 前缀)
  assert.strictEqual(result.latestVersion, "0.1.6-dev.2");
});

test("R27 checkForUpdate latestVersionDisplay 与 latestVersion 同步时返回一致 (无 v 输入)", async () => {
  const release = makeRelease({ tag_name: "v2.0.0" });
  const { svc } = makeFakeApp({ version: "1.0.0", release });
  const result = await svc.checkForUpdate();
