...
const UPDATE_SERVICE_PATH = path.join(__dirname, '..', '..', 'electron', 'src', 'main', 'services', 'UpdateService.js');
const { UpdateService, normalizeUpdateError, parseSha256FromBody, computeFileSha256 } = require(UPDATE_SERVICE_PATH);
const VERSION_COMPARE_PATH = path.join(__dirname, '..', '..', 'electron', 'src', 'main', 'utils', 'versionCompare.js');
    ...
    assets: opts.assets || [
      {
        ...
        browser_download_url: 'https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/2.0.0.exe',
        size: 50000000,
      },
    ],
  };
...
test('P0-4 sanitizeUpdateFileName: 剥离路径成分 + 扩展名白名单', () => {
  const { sanitizeUpdateFileName } = require(
    path.join(__dirname, '..', '..', 'electron', 'src', 'main', 'services', 'UpdateService.js')
  );
...
test('P0-4 isTrustedDownloadUrl: 仅放行 GitHub release 域', () => {
  ...
  assert.strictEqual(
    isTrustedDownloadUrl('https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/app.exe'),
    true
  );
...
test('P3-2 两次 checkForUpdate 不同 asset → 各自 hash 按文件名保留', async () => {
  ...
  const releaseFull = makeRelease({
    assets: [
      {
        name: 'XKAutoTester Setup v2.0.0.exe',
        browser_download_url: 'https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/full.exe',
        size: 1,
      },
    ],
    body: `Release notes\nSHA256: ${hashFull}`,
    ...
    assets: [
      {
        ...
        browser_download_url: 'https://github.com/RingOnTheWay/XKAutoTester/releases/download/v2.0.0/lite.exe',
        size: 1,
      },
    ],
    body: `Release notes\nSHA256: ${hashLite}`,
  ...
  assert.strictEqual(svc._getExpectedSha256('XKAutoTester Setup v2.0.0.exe'), hashFull, '完整包 hash 应按文件名保留');
  assert.strictEqual(svc._getExpectedSha256('XKAutoTester Setup v2.0.0 Lite.exe'), hashLite, 'Lite 包 hash 已绑定');
});
...
test('P2-3 installUpdate updateDir 外路径 → 拒绝 + 不调 installStrategy', async () => {
  ...
  await assert.rejects(svc.installUpdate(outsidePath), /outside update directory/);
...
test('R27 normalizeUpdateError fetch cause 内 SSL 错误 → ssl_failed (非 unknown)', () => {
  ...
  raw.cause = {
    errors: [{ code: 'UNABLE_TO_VERIFY_LEAF_SIGNATURE', message: 'unable to verify' }],
  };
  const result = normalizeUpdateError(raw);
...
test('R27 cancelDownload 无活跃下载幂等成功 (state=no_active)', async () => {
  ...
  assert.strictEqual(result.state, 'no_active', '无活跃应标 no_active (权威词汇 state, 非错误)');
});
...
test('R27 calcProgressPercent 保留 1 位小数 (中间进度有小数)', async () => {
  ...
  assert.strictEqual(calcProgressPercent(1, 1000), 0.1, '千分之一 = 0.1% (非 0)');
