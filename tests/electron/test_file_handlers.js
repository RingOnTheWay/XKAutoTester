...
const HANDLER_PATH = path.resolve(__dirname, '../../electron/src/main/handlers/fileHandlers.js');
...
describe('fileHandlers CREATE_DIRECTORY 路径安全', () => {
  ...
  test('系统关键目录 (C:\\Windows) → 拒绝', async () => {
    const ipc = makeIpc();
    if (process.platform === 'win32') {
      const r = await ipc.invoke('createDirectory', 'C:\\Windows\\System32\\xkat-test-ghost');
  ...
  test('非字符串 / 空串 → 拒绝', async () => {
    const ipc = makeIpc();
    assert.strictEqual((await ipc.invoke('createDirectory', 123)).success, false);
    assert.strictEqual((await ipc.invoke('createDirectory', '')).success, false);
    assert.strictEqual((await ipc.invoke('createDirectory', null)).success, false);
  });
